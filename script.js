...
// initialize coordinates display visibility
...
// reset points button handler (already defined, just clarified name in HTML)
if (resetBtn) {
...
// helper: convert screen (canvas pixels) -> world (untransformed) coords
...
// helper: world bounds visible in canvas
...
// keyboard handler to enable panning with spacebar
...
canvas.addEventListener('pointerdown', (e) => {
    // start panning if middle button or space is held
...
canvas.addEventListener('pointermove', (e) => {
    if (isPanning) {
        ...
        panX = startPanX + dx;
        panY = startPanY + dy;
        return; // don't update control points while panning
    }
    // otherwise, let mouse movement control curve targets (handled below)
});
...
// initControlPoints is called after the first resize so sizes are correct
...
/* =========================================================
   MOUSE INPUT
========================================================= */
// pointermove handler now handles both panning and mouse->world mapping
canvas.addEventListener('pointermove', (e) => {
    if (isPanning) return; // panning handled elsewhere

    const rect = canvas.getBoundingClientRect();
    const sx = e.clientX - rect.left;
    const sy = e.clientY - rect.top;

    // store raw screen coords for display
    lastMouseX = sx;
    lastMouseY = sy;

    // convert to world coords (accounting for pan & zoom)
    const world = screenToWorld(sx, sy);

    // offsets in world space (so they scale with zoom)
    ...
    // compute visible world bounds and apply margin (also in world units)
...
// double-click handler: set P1/P2 targets at clicked world coords
...
// mousewheel zoom with Ctrl key
canvas.addEventListener('wheel', (e) => {
    if (e.ctrlKey) {
        e.preventDefault();
        const zoomDelta = e.deltaY > 0 ? 0.9 : 1.1; // scroll down = zoom out
...
function drawCoordinates() {
    ...
    // draw coordinate labels near control points
    ctx.fillStyle = "#0f0";
    ...
    // update coordinates display at bottom-left
...
function animate() {
    ...
    // translate to center + pan, then scale around center
    ...
    // update FPS (smoothed) and display if enabled
